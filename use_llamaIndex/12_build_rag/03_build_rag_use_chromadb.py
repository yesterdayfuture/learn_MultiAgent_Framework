...
建立 rag,使用chromadb 持久化保存已经向量化后的数据
...
# # 在 nodes 上建立索引，并放入向量库
# vec_index = VectorStoreIndex(nodes=cur_nodes, embed_model=embedding_model, show_progress= True, storage_context=storage_context)

# 如果数据已经存入向量库，则不需要再次建立索引，直接从向量库加载索引
...
restriver = vec_index.as_retriever()
response = restriver.retrieve(
    "介绍一下粮食玉米"
)
print("*"*50)
